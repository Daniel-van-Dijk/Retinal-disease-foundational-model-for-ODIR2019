...
validation_dataset = ODIRDataset(validation_df, 'data/cropped_ODIR-5K_Training_Dataset', transforms=val_transform)
...
class VGG(nn.Module):
    def __init__(self, num_diseases=8):
        ...
        self.fc1 = nn.Linear(512 * 7 * 7 * 2, 4096) 
        self.fc2 = nn.Linear(4096, 4096)
        self.fc3 = nn.Linear(4096, 8)  

    def forward(self, left, right):
        left = self.vgg16(left)
        right = self.vgg16(right)
...
df_diseases = df[disease_columns]
samples = df.shape[0]

class_weights = list((samples - np.sum(df_diseases, axis=0)) / samples)
print(class_weights)
...
def train(model, num_epochs, train_dataloader, validation_dataloader):
    ...
    for epoch in range(num_epochs):
        ...
        print('Avg train loss: ', train_loss / len(train_dataloader))
...
train(model, 75, train_dataloader, validation_dataloader)
